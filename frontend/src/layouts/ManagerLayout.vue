...
export default {
  name: 'ManagerLayout',
  components: { AppSidebar, AppNavbar, SlackChat, ChatBubbleLeftRightIcon, XMarkIcon },
  setup() {
    ...
    const goToTickets = () => router.push('/manager/tickets')

    const fetchPendingTicketsCount = async () => {
      try {
        const res = await fetch('/api/tickets/count?status=pending&manager_id=' + authStore.user?.id, {
    ...
    return {
      ...
      showChatModal, chatInterface, chatKey, pendingTicketsCount, unreadChatCount,
      handleNavClick, openChatModal, closeChatModal, updateUnreadCount, goToTickets,
    }
  },
}
</script>
