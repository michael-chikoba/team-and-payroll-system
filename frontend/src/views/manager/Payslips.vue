      ...
      <div class="user-greeting">
        <div class="avatar-section">
          ...
          <div class="user-info">
            ...
            <div class="role-meta">
              <span class="role-badge" v-if="isManagerOrAdmin">{{ userRoleLabel }} View</span>
              <span class="role-badge employee" v-else>Employee View</span>
            </div>
          </div>
        </div>

        <div class="header-actions">
          <button @click="fetchPayslips" class="btn-outline" title="Refresh Data">
          ...
          <button v-if="filterMonth || filterStatus || filterEmployee" @click="resetFilters" class="btn-outline warning">
            Clear Filters
          </button>
        </div>
      </div>
    ...
    <div class="dashboard-content">
      ...
      <div class="metrics-section" v-if="!loading && payslips.length > 0">
        ...
        <div class="metrics-grid">
          ...
          <div v-if="isManagerOrAdmin" class="metric-card" style="--accent:#3b82f6;">
      ...
      <div class="table-section">
        ...
        <div v-else class="table-container">
          <div class="list-header">
            <div v-if="isManagerOrAdmin" class="col-employee">Employee</div>
          ...
          <div v-for="payslip in paginatedPayslips" :key="payslip.id" class="list-row" @click="viewPayslip(payslip)">
            <div v-if="isManagerOrAdmin" class="col-employee">
              <div class="member-avatar">{{ getEmployeeInitials(payslip) }}</div>
              <div class="name-info">
                <span class="emp-name">{{ getEmployeeName(payslip) }}</span>
                <span class="emp-sub" v-if="getEmployeeId(payslip) !== 'N/A'">ID: {{ getEmployeeId(payslip) }}</span>
              </div>
            </div>
            <div class="col-period">
              <span class="period-text">{{ formatPeriodShort(payslip) }}</span>
            </div>
            <div class="col-amount text-right font-mono">{{ formatCurrency(payslip.grossPay || payslip.gross_pay) }}</div>
            <div class="col-amount text-right font-mono text-danger">{{ formatCurrency(payslip.deductions || payslip.total_deductions) }}</div>
            <div class="col-amount text-right font-mono text-success font-bold">{{ formatCurrency(payslip.netPay || payslip.net_pay) }}</div>
            <div class="col-status">
              <span :class="['status-badge', getStatusClass(payslip.status)]">
                <span class="dot"></span>{{ formatStatus(payslip.status) }}
              </span>
            </div>
            <div class="col-date text-muted">{{ formatDate(payslip.payment_date || payslip.paymentDate) }}</div>
        ...
        <div class="modal-container">
          <div class="modal-header">
            <div class="modal-title-wrap">
              ...
              <div>
                ...
                <p class="modal-position">Castle Holding Zambia · {{ formatPeriod(selectedPayslip) }}</p>
              </div>
            </div>
          ...
          <div class="modal-body">
            ...
            <div class="modal-stats">
              <div class="modal-stat" v-if="isManagerOrAdmin">
                <small>Employee</small>
                <strong>{{ getEmployeeName(selectedPayslip) }}</strong>
                <span v-if="getEmployeePosition(selectedPayslip) !== 'N/A'" class="sub">{{ getEmployeePosition(selectedPayslip) }}</span>
              </div>
              <div class="modal-stat">
                <small>Payment Date</small>
                <strong>{{ getPaymentDate(selectedPayslip) }}</strong>
              </div>
              <div class="modal-stat">
                <small>Status</small>
            ...
            <div class="split-view">
              <div class="detail-column">
                ...
                <div class="line-items">
                  ...
                  <div class="line-item" v-if="(selectedPayslip.overtime_pay || selectedPayslip.overtimePay) > 0"><span>Overtime Pay</span><span>{{ formatCurrency(selectedPayslip.overtime_pay || selectedPayslip.overtimePay || 0) }}</span></div>
                  <div class="line-item" v-if="(selectedPayslip.other_allowances || selectedPayslip.otherAllowances) > 0"><span>Other Allowances</span><span>{{ formatCurrency(selectedPayslip.other_allowances || selectedPayslip.otherAllowances || 0) }}</span></div>
                </div>
                <div class="col-total">
                  <span>Gross Pay</span>
              ...
              <div class="detail-column">
                ...
                <div class="line-items">
                  ...
                  <div class="line-item" v-if="(selectedPayslip.other_deductions || selectedPayslip.otherDeductions) > 0"><span>Other Deductions</span><span class="text-danger">{{ formatCurrency(selectedPayslip.other_deductions || selectedPayslip.otherDeductions || 0) }}</span></div>
                </div>
                <div class="col-total">
            ...
            <p class="generated-note">Generated on {{ formatDate(selectedPayslip.created_at) }}</p>
          </div>

          <div class="modal-footer">
            <button @click="closeModal" class="btn-secondary">Close</button>
            <button v-if="isManagerOrAdmin && selectedPayslip.status === 'draft'" @click="approvePayslip(selectedPayslip)" class="btn-approve">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"></polyline></svg>
              Approve
            </button>
...
export default {
  ...
  computed: {
    userRole() { return this.authStore.user?.role?.toLowerCase() || 'employee' },
    isManagerOrAdmin() { return this.userRole === 'manager' || this.userRole === 'admin' },
    userRoleLabel() { return { admin: 'Admin', manager: 'Manager', employee: 'Employee' }[this.userRole] || 'Employee' },
    totalEarnings() { return this.payslips.reduce((sum, p) => sum + (p.netPay || p.net_pay || 0), 0) },
    latestPayslipPeriod() { return this.payslips.length ? this.formatPeriodShort(this.payslips[0]) || '—' : '—' },
    paginatedPayslips() {
      const start = (this.currentPage - 1) * this.perPage
      return this.payslips.slice(start, start + this.perPage)
    },
    totalPages() { return Math.ceil(this.payslips.length / this.perPage) },
    apiBasePath() {
      if (this.userRole === 'manager') return '/api/manager/payslips'
      if (this.userRole === 'admin') return '/api/admin/payslips'
      return '/api/employee/payslips'
    },
    employeesApiPath() {
      if (this.userRole === 'manager') return '/api/manager/employees'
      if (this.userRole === 'admin') return '/api/admin/employees'
      return null
    }
  },
  ...
  methods: {
    initializePage() {
      if (this.userRole === 'admin') this.pageName = 'All Payslips'
      else if (this.userRole === 'manager') this.pageName = 'Team Payslips'
      this.fetchPayslips()
      if (this.isManagerOrAdmin) this.fetchEmployees()
    },
    async fetchPayslips() {
      ...
      try {
        ...
        if (this.payslips.length > 0) {
          const years = [...new Set(this.payslips.map(p => {
            const d = new Date(p.pay_period_start || p.period_start || p.created_at || p.payment_date)
    ...
    async fetchEmployees() {
      ...
      try {
        const r = await axios.get(this.employeesApiPath)
        this.employees = r.data.data || r.data.employees || r.data || []
      } catch {}
    },
    async downloadPayslip(payslip) {
      const id = payslip.id || payslip.payslip_id
      if (!id) { this.showToast('Invalid payslip ID', 'error'); return }
      this.downloading = { ...this.downloading, [id]: true }
      try {
        let response
        try {
          response = await axios.get(`${this.apiBasePath}/${id}/download`, { responseType: 'blob', headers: { Accept: 'application/pdf' } })
        } catch {
          for (const ep of [`/api/payslips/${id}/download`, `/api/download-payslip/${id}`, `/api/employee/payslips/${id}/download`]) {
            try { response = await axios.get(ep, { responseType: 'blob', headers: { Accept: 'application/pdf' } }); break } catch {}
          }
        }
        ...
        link.href = url
        const period = String(payslip.period || id).replace(/\s+/g, '-')
        const empName = this.isManagerOrAdmin ? `-${this.getEmployeeName(payslip).replace(/\s+/g, '-')}` : ''
        link.setAttribute('download', `Castle-Payslip-${period}${empName}.pdf`.toLowerCase())
    ...
    async viewPayslip(payslip) {
      try {
        const id = payslip.id || payslip.payslip_id
        const r = await axios.get(`${this.apiBasePath}/${id}`)
        this.selectedPayslip = { ...payslip, ...(r.data.data || r.data), id }
      } catch { this.selectedPayslip = { ...payslip, id: payslip.id || payslip.payslip_id } }
    },
    async approvePayslip(payslip) {
      if (!confirm('Approve this payslip?')) return
      try {
        const id = payslip.id || payslip.payslip_id
        await axios.post(`${this.apiBasePath}/${id}/approve`)
        this.showToast('Payslip approved!', 'success')
        this.fetchPayslips()
        if (this.selectedPayslip?.id === id) this.selectedPayslip.status = 'generated'
      } catch { this.showToast('Failed to approve payslip.', 'error') }
    },
    getEmployeeName(p) { return p.employee?.name || p.employee?.full_name || (p.employee?.first_name ? `${p.employee.first_name} ${p.employee.last_name || ''}`.trim() : '') || p.employee_name || 'Unknown' },
    getEmployeeInitials(p) { const n = this.getEmployeeName(p); return n === 'Unknown' ? '?' : n.split(' ').map(x => x[0]).slice(0,2).join('').toUpperCase() },
    getEmployeeId(p) { return p.employee?.employee_id || p.employee?.id || 'N/A' },
    getEmployeePosition(p) { return p.employee?.position || 'N/A' },
    closeModal() { this.selectedPayslip = null },
    resetFilters() { this.filterYear = new Date().getFullYear(); this.filterMonth = ''; this.filterStatus = ''; this.filterEmployee = ''; this.fetchPayslips() },
    ...
    handleApiError(err) {
      ...
      else this.error = err.response?.data?.message || 'Unable to load data.'
    },
    ...
    formatStatus(s) { return { generated: 'Generated', paid: 'Paid', draft: 'Draft', processing: 'Processing', pending: 'Pending' }[s?.toLowerCase()] || s || 'Unknown' },
    getStatusClass(s) { return { generated: 'warning', paid: 'success', draft: 'danger', processing: 'warning', pending: 'danger', completed: 'success' }[s?.toLowerCase()] || 'neutral' },
    formatDate(date) {
      if (!date) return '—'
      try { const d = new Date(date); return isNaN(d.getTime()) ? String(date) : d.toLocaleDateString('en-ZM', { year: 'numeric', month: 'short', day: 'numeric' }) } catch { return '—' }
    },
    formatPeriod(payslip) {
      if (!payslip?.period) return 'N/A'
      let p = payslip.period
      if (typeof p === 'string' && p.trim().startsWith('{')) { try { p = JSON.parse(p) } catch { return p } }
      if (typeof p === 'object' && p !== null) {
        const s = p.start ? this.formatDate(p.start) : ''; const e = p.end ? this.formatDate(p.end) : ''
        return s && e ? `${s} – ${e}` : s || 'N/A'
      }
      const str = String(p)
      if (/^\d{4}-\d{2}$/.test(str)) {
        const [y, m] = str.split('-')
    ...
    formatPeriodShort(p) { return this.formatPeriod(p) },
    getPaymentDate(p) {
      if (p.payment_date) return this.formatDate(p.payment_date)
      if (typeof p.period === 'string' && p.period.startsWith('{')) { try { const parsed = JSON.parse(p.period); if (parsed.payment_date) return this.formatDate(parsed.payment_date) } catch {} }
      return '—'
    },
...
.role-badge.employee { background: #f0fdf4; border-color: #bbf7d0; color: #166534; }
...
.btn-approve {
  display: flex; align-items: center; gap: 0.4rem;
  padding: 0.5rem 1.25rem; background: #10b981; color: white;
...
.records-count { font-size: 0.78rem; font-weight: 700; color: #64748b; background: #f1f5f9; padding: 0.2rem 0.7rem; border-radius: 9999px; white-space: nowrap; }
...
.list-row {
  display: grid;
  grid-template-columns: 2fr 1.2fr 1fr 1fr 1fr 0.9fr 1fr 0.8fr;
...
/* No-manager layout (fewer columns) */
.list-header:not(:has(.col-employee)),
.list-row:not(:has(.col-employee)) {
  grid-template-columns: 1.5fr 1fr 1fr 1fr 0.9fr 1fr 0.8fr;
}

.col-employee { display: flex; align-items: center; gap: 0.75rem; }
...
.emp-sub { font-size: 0.7rem; color: #64748b; }
...
.modal-container {
  background: white; width: 100%; max-width: 780px; max-height: 90vh;
  border-radius: 16px;
...
.modal-stat .sub { font-size: 0.72rem; color: #64748b; }
...
.col-total {
  ...
  border-top: 1px solid #e2e8f0; padding-top: 0.75rem; margin-top: 0.5rem;
  font-size: 0.875rem;
}
...
.generated-note { font-size: 0.75rem; color: #94a3b8; font-style: italic; text-align: center; margin: 0; }
...
@media (max-width: 1000px) {
  .list-header, .list-row { grid-template-columns: 2fr 1fr 1fr 1fr 0.9fr 0.8fr; }
  .col-employee { display: flex; }
...
@media (max-width: 768px) {
  ...
  .list-row { grid-template-columns: 1fr auto; grid-template-areas: "name status" "amounts actions"; padding: 1rem; gap: 0.5rem; }
  .col-employee, .col-period { grid-area: name; }
