<template>
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="$emit('close')"></div>
      <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>

      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
        <form @submit.prevent="handleAccept">
          <!-- Header -->
          <div class="bg-green-600 px-6 py-4">
            <h3 class="text-lg font-medium text-white">Accept Invitation</h3>
          </div>

          <!-- Content -->
          <div class="bg-white px-6 py-6">
            <div class="space-y-4">
              <!-- Invitation Info -->
              <div class="bg-gray-50 p-4 rounded-lg">
                <p class="text-sm text-gray-700">
                  You are about to accept the invitation to join:
                </p>
                <p class="mt-2 text-lg font-semibold text-gray-900">
                  {{ invitation.business_group.name }}
                </p>
                <p class="text-sm text-gray-500">
                  as {{ invitation.proposed_role }}
                </p>
              </div>

              <!-- Optional Message -->
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  Message (Optional)
                </label>
                <textarea
                  v-model="message"
                  rows="3"
                  placeholder="Add a message..."
                  class="block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm"
                ></textarea>
              </div>

              <!-- Error Display -->
              <div v-if="error" class="rounded-md bg-red-50 p-4">
                <p class="text-sm text-red-800">{{ error }}</p>
              </div>
            </div>
          </div>

          <!-- Footer -->
          <div class="bg-gray-50 px-6 py-4 flex flex-row-reverse gap-3">
            <button
              type="submit"
              :disabled="accepting"
              class="inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-sm font-medium text-white hover:bg-green-700 focus:outline-none disabled:opacity-50"
            >
              {{ accepting ? 'Accepting...' : 'Accept Invitation' }}
            </button>
            <button
              type="button"
              @click="$emit('close')"
              :disabled="accepting"
              class="inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none disabled:opacity-50"
            >
              Cancel
            </button>
          </div>
        </form>
      </div>
    </div>
  </div>
</template>

<script>
import { ref } from 'vue'
import axios from 'axios'

export default {
  name: 'AcceptInvitationModal',
  
  props: {
    invitation: {
      type: Object,
      required: true
    }
  },
  
  emits: ['close', 'accepted'],
  
  setup(props, { emit }) {
    const accepting = ref(false)
    const message = ref('')
    const error = ref(null)

    const handleAccept = async () => {
      accepting.value = true
      error.value = null

      try {
        const response = await axios.post(
          `/api/business-group-invitations/${props.invitation.id}/accept`,
          { message: message.value }
        )

        if (response.data.success) {
          emit('accepted')
        }
      } catch (err) {
        error.value = err.response?.data?.message || 'Failed to accept invitation'
      } finally {
        accepting.value = false
      }
    }

    return {
      accepting,
      message,
      error,
      handleAccept
    }
  }
}
</script>