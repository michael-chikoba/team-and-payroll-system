<!-- resources/js/components/BusinessGroups/GroupSettingsModal.vue -->
<template>
...
export default {
  ...
  setup(props, { emit }) {
    const form = ref({
      ...
      description: '',
      group_type: '',
      allow_cross_business_tickets: false,
      allow_cross_business_tasks: false,
    ...
    onMounted(() => {
      Object.assign(form.value, props.group);
    });
    ...
    const handleSubmit = async () => {
      try {
        ...
        const response = await axios.put(`/api/business-groups/${props.group.id}`, form.value);
        ...
        errorMessage.value = error.response?.data?.message || 'Failed to update business group';
      } finally {
